...
import { normalizeEventPostsCSV } from "@/shared/utils/normalizeCSV";
import { handleEventPostsCSVDownload } from "@/shared/utils/handleCSVDownload";
...
const CSVUploadForm = ({ nextStep, isFirstStep }) => {
  ...
  const importEventPostsData = useEventPostsStore(
    (state) => state.importEventPostsData,
  );

  const handleCSVParsed = (row) => {
    try {
      const normalized = normalizeEventPostsCSV(row);
      importEventPostsData(normalized);
    ...
    <FormContainer>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <ButtonGroup
          ...
          handleCSVDownload={handleEventPostsCSVDownload}
        />
      </form>
    </FormContainer>
  );
