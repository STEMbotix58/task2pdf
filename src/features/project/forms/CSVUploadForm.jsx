...
import { normalizeProjectCSV } from "@/shared/utils/normalizeCSV";
import { handleProjectCSVDownload } from "@/shared/utils/handleCSVDownload";
...
const CSVUploadForm = ({ nextStep, isFirstStep }) => {
  ...
  const handleCSVParsed = (row) => {
    try {
      const normalized = normalizeProjectCSV(row);
      importProjectData(normalized);
    ...
    <FormContainer>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <ButtonGroup
          ...
          handleCSVDownload={handleProjectCSVDownload}
        />
      </form>
    </FormContainer>
  );
